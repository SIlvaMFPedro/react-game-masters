...
import {BrowserRouter, Routes, Route, useNavigate, useLocation, useLoaderData} from "react-router-dom";
...
function App() {
  ...
  if (location.pathname !== "/game-masters/" && location.pathname !== "/game-masters/browse" && selectedGame === false){
  ...
  const handleSearchSubmit = (e) => {
    ...
    e.preventDefault();
    setSearching(true);
    if (location.pathname !== "/game-masters/browse"){
      navigate('/game-masters/browse');
    }
  }
  ...
  const handleSelectGame = (e) => {
    ...
    else if(e.target.classList[0] !== "AddToCart_addToCart_zbJPe"){
      setSelectedGame(games[e.target.parentNode.id]);
      navigate(`/game-masters/games/${games[e.target.parentNode.id].surname}`);
    }
  }
  ...
  const openGamePage = (e) => {
    ...
    navigate(`/game-masters/games/${selectedGameSurname}`);
  }
  ...
  const handleHoverGame = (e) => {
    let handleHoveredGame = allGames.map((game, i) => {
      if (e.target.id === i){
  ...
  useEffect(() => {
    setOverlap(false);
    if (location.pathname === "/game-masters/"){
      setBrowsing(false);
    }
    else{
      setBrowsing(true);
    ...
    else if (location.pathname === "/game-masters/browse"){
  ...
  return (
    <AnimatePresence exitBeforeEnter>
      <Routes key={location.pathname} location={location}>
        ...
        <Route path="/game-masters/browse"
               element={<Browse
                 cart={cart}
                 cartAmount={cartAmount}
                 handleHover={handleHover}
                 ...
                 setHoverState={setHoverState}
                 openGamePage={openGamePage}/>}/>
        <Route path="/game-masters/games/:gameId"
               element={<GamePage
                   cart={cart}
                   cartAmount={cartAmount}
                   handleHover={handleHover}
